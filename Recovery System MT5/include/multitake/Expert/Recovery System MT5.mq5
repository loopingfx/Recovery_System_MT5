...
//+------------------------------------------------------------------+
//
sinput string Settings_bworkgroup_1="           MAIN RECOVERY AND LAUNCH SETTINGS";//Chapter I "Main Recovery And Launch Settings"
input launch_mode Work_Mode=Start_at_drawndown_in_money;//Mothod of Launching
input double Drawdown_for_Start=200;//Seting Drawdown in percent or money
input group_for Work_With=All_orders_of_current_symbol;//Specify of working order group
input start_profit_from OrdersSelector=Start_with_the_farest_order_to_profitability;
input calculation_method RiskManagment=Manual_calculation;//Method of auto calculation
input int RecoverMagicNumber=123454321;//Magic Numver for Recovery orders
input int HedgeMagicNumber=1122332211;//Magic Number for Locking orders
input order_open_method  TypeOfZones=Sell_zone_is_above_and_buy_zone_is_below;//Specify type of trade
input int VarSizeOfZone=1;//Size of start buyzone and sellzone in point (0=Auto)
input bool Locking=true;//Allow locking during recovery process active
input string LockedOrderComment="lock order";//Identify for locking order
sinput string Settings_bworkgroup_2="           RECOVERY ORDERS SETTINGS";//Chapter II "Recovery Orders Settings"
input bool CanTradeBothSides=false;//Allow to trade in both size for Without Filter Mode
input order_type_filter RecTypeOpen=Buy_and_Sell_orders;//Type of order
input indicator_filter Trend_Filter_Grids=Filtering_via_Two_MAs;//Indicator Filter
input double MinRecStartLots=0.02;//Start volume size of recovery order
input double Multiplier=1.3;//Multiplier factor for volume of recovery orders
input int Start_Step_Grids=50;//Start Step for restore orders
input double Step_Multiplier=1.2;//Multipiler factor for Step between recovert orders
sinput string Settings_bworkgroup_3="           TRAILING AND TAKEPROFIT SETTINGS";//Chapter III "Trailing And Takeprofit Settings"
input double MinPart_For_Close=0.02;//Minimum order size to be processed 
input double TakeProfit_Closure_Money=5;//Profit Taking level in Money for Group orders
input double TakeProfit_System_Money=1000;//Profit Taking level in Money for all orders
bool OverLap_LH=false;
int Overlap_after=4;
sinput string Settings_bworkgroup_4="           NOTIFICATIONS SETTINGS";//Chapter IV "Notifications Settings"
input bool Send_Push_Notifications=false;
input bool Send_Mail=false;
input bool Send_PopUp_Alerts=false;
sinput string Settings_bworkgroup_5="           PROTECTION SETTINGS";//Chapter V "Protection Settings"
bool Close_Another_Charts=true;
bool One_order_per_bar=true;
input bool Delete_SL_TP=true;//Delete Stoploss and Takeprofit when locking
bool Close_Profit_At_Start=true;
bool Delete_Pendings_At_Start=true;
bool TrueECN_Type_of_account=false;
input int Slippage=30;//Slippage Limit
input int Spread_Limit=75;//Spread Limit
input double Maximum_Order_Size=2;//Maximum Size of Recovery Orders
input int Maximum_Orders_In_Work=100;//Maximum Limit of order in portforio
input string RecoveryComment="recover order";//Identify to Recovery Orders
input bool Use_Addictional_Magic=false;//Use The Addition Magic Number
input int Addictional_Magic=543212345;//Magic Number for Addition to be group
sinput string Settings_bworkgroup_6="           AUTOCALCULATIONS SETTINGS";//Chapter VI "Autocalculations Settings"
input double MaxRecStartLots=10;//Maximum Size of Restoring Orders at Start
input double MaxPartForClose=0.10;//Maximum Volume for Partial Closing
input int Maximum_Step_between_Ord=2000;//Maximum Step between Recovery Orders
input int Minimum_Step_between_Ord=50;//Minimum Step between Recovery Orders
input double  MultiplierFactorToClosureLots=4;;//Multilplier to calculate Closure from Recovery Orders Size
input double  MinClosureStartLots=0.05;//Minimum size for the Closure
input double  MaxClosureStartLots=10;//Maximum size for the Closure
selection_method Closures_Type=Close_more_loss;
input double QOpen_Lots=0.5;//Volume Size for Deal Button And Tester
order_group Orders_Type_DealButtons=Locked_group_of_orders;
input int DealButtonMagicNumber=455;//Magic Number for Deal Button 
sinput string Settings_bworkgroup_8="           GRAPHICS SETTINGS";//Chapter VII "Graphics Settings"
input scheme_apply Color_Scheme=Default_Scheme;//Panel Color Scheme
input int  FontSize=8;//Panel Font Size
input bool ShowPanels=true;//Panel Showing
input bool RightDealButtonPanel=false;//Display Deal Button Panel On The Right of Terminal
sinput string SAR_Settings="           INDICATORS - PARABOLIC SAR SETTINGS";//Chapter VIII "Indicators - Parabolic SAR Settings"
input ENUM_TIMEFRAMES SAR_Timeframe=PERIOD_CURRENT;//Indicator Parabolic SAR Timeframe
input double step_SAR=0.02;
input double maximum_SAR=0.2;
sinput string Alligator_Settings="           INDICATORS - ALLIGATOR SETTINGS";//Chapter IX "Indicators - Alligator Settings"
input ENUM_TIMEFRAMES Alligator_Timeframe=PERIOD_CURRENT;//Indicator Alligator Timeframe
...
sinput string TwoMA_Settings="           INDICATORS - 2MA SETTINGS";//Chapter X "Indicators - 2MA Settings"
input ENUM_TIMEFRAMES MAS_Timeframe=PERIOD_M5;//Indicator 2 Moving Average Timeframe
...
sinput string HAshi_Settings="           INDICATORS - HEIKEN ASHI SETTINGS";//Chapter XI "Indicators - Heiken Ashi Settings"
input ENUM_TIMEFRAMES HARSI_Timeframe=PERIOD_CURRENT;//Indicator Heiken Ashi Timeframe
//---
//+------------------------------------------------------------------+
...
//  MainRecoveryAndLaunchSetting.RiskManagment=RiskManagment;
...
//  TrailingAndTakeProfitSetting.OverLap_LH=OverLap_LH;
// TrailingAndTakeProfitSetting.Overlap_after=Overlap_after;
//---
...
//--
// ProtectionSetting.Close_Another_Charts=Close_Another_Charts;
//  ProtectionSetting.One_order_per_bar=One_order_per_bar;
   ProtectionSetting.Delete_SL_TP=Delete_SL_TP;
//  ProtectionSetting.Close_Profit_At_Start=Close_Profit_At_Start;
// ProtectionSetting.Delete_Pendings_At_Start=Delete_Pendings_At_Start;
//  ProtectionSetting.TrueECN_Type_of_account=TrueECN_Type_of_account;
...
// AutoCalcuationSetting.Closures_Type=Closures_Type;
   AutoCalcuationSetting.QOpen_Lots=QOpen_Lots;

//  AutoCalcuationSetting.Orders_Type_DealButtons=Orders_Type_DealButtons;
   AutoCalcuationSetting.DealButtonMagicNumber=33434;
//---
   ...
   if(data_structure.recovery_turn_on)
     {
      ...
      if(AccountInfoInteger(ACCOUNT_TRADE_MODE)!=ACCOUNT_TRADE_MODE_DEMO) ExpertRemove();
     }
//--
   if(data_structure.recovery_on_service)
     {
      ...
      if(!structure.recovery_on_service && !structure.recovery_turn_on)
        {
         ...
         if(notify_setting.Send_PopUp_Alerts) SendNotification(messege);
...
void SetGroupPanel(trade_info &info)
  {
   string curr_=AccountInfoString(ACCOUNT_CURRENCY); // memory to show in panel
//--- show to panel 0 line 1
